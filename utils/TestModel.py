...
def test_precess(MODEL, pbar, LOSS, DEVICE, FOLD_NUM):
    ...
    plt.figure()
    lw = 2
    plt.plot(fpr1, tpr1, color='blue', lw=lw, label='ROC curve(area = %0.2f)' % roc_auc)
    ...
    plt.xlim([-0.05,1.05])
    plt.ylim([-0.05,1.05])
    plt.step(fpr,tpr,color='blue',label='(PRC={:.4f})'.format(prc))
    plt.plot([0,1],[1,0],color='b',linestyle='--')
    plt.legend(loc='lower right')
    plt.savefig('PR.png')
    plt.show()
    ...
    return Y, P, test_loss, F1, ACC, Sensitivity, Specificity, MCC
...
def test_model(MODEL, dataset_loader, save_path, DATASET, LOSS, DEVICE, dataset_class="Train", save=True, FOLD_NUM=1):
    ...
    T, P, loss_test, F1_test, ACC_test, Sensitivity_test, Specificity_test, MCC_test = test_precess(
    ...
    results = '{}: Loss:{:.5f};F1:{:.5f};ACC:{:.5f};Sensitivity:{:.5f};Specificity:{:.5f};MCC:{:.5f}.' \
        .format(dataset_class, loss_test, F1_test, ACC_test, Sensitivity_test, Specificity_test, MCC_test)
    print(results)
    return results, F1_test, ACC_test, Sensitivity_test, Specificity_test, MCC_test
